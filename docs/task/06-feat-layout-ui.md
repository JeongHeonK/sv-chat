...
미인증 사용자는 `/login`으로 redirect하는 인증 가드를 `(app)` route group에 추가.

## DoD (Definition of Done)

### RED — 실패하는 테스트 작성
- [ ] 미인증 사용자 `/` 접근 시 `/login`으로 redirect 테스트 (Red)
- [ ] 데스크탑 뷰포트: Sidebar + Main 분할 영역 존재 테스트 (Red)
- [ ] 모바일 뷰포트: 단일 뷰 구조 테스트 (Red)
- [ ] GNB 상단바 렌더링 테스트 (Red)

### GREEN — 테스트를 통과하는 최소 구현
- [ ] `(app)/+layout.server.ts`에서 세션 체크 + redirect(303, '/login') 구현
- [ ] GNB 상단바 컴포넌트 구현
...
- [ ] 반응형 breakpoint 상수화
- [ ] `(auth)` 그룹에 인증 사용자 → `/` redirect 추가 검토
