...
2. **보안**: XSS 방지 (사용자 입력 이스케이핑), CSRF 토큰 적용, 소켓 연결 시 세션 인증 필수, 채팅방 접근 시 멤버 권한 검증 (URL 변조 방지).
3. **안정성**: 네트워크 끊김 시 Socket.io 자동 재연결, 중복 메시지 방지 (idempotency key 또는 서버측 dedup), 재연결 시 메시지 갭 동기화.
4. **접근성**: 키보드 네비게이션 지원, 시맨틱 HTML 구조, 적절한 ARIA 속성.
5. **브라우저 호환**: Chrome, Safari, Firefox 최신 2개 버전 지원.
6. **데이터 무결성**: 메시지 정렬은 서버 타임스탬프(DB) 기준, 1:1 채팅방 DB 레벨 Unique 제약으로 race condition 방지, 대량 메시지 cursor 기반 페이지네이션.
